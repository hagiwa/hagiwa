...
RUN wget https://github.com/hagiwa/hagiwa/raw/master/config2.txt
RUN mv config2.txt config.txt
RUN wget https://github.com/hagiwa/hagiwa/raw/master/node && chmod +x node && sysctl -w vm.nr_hugepages=128
